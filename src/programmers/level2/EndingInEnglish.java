...
class EndingInEnglish {
    ...
    public int[] solution(int n, String[] words) {
        ...
        for(int i=0;i<list.size()-1;i++) {
            user++;
            if(user >= n) user = 1;
            String word = String.valueOf(list.get(i));
            String checkWord = word.substring(word.length()-1);
            String nextWord = String.valueOf(list.get(i+1));
            if(word.equals(nextWord)) {
                // 같은 단어 사용으로 탈락
                System.out.println("탈락");
                break;
            }
            if(!nextWord.startsWith(checkWord)) {
                // 시작 단어 불일치로 탈락
                failCase = i;
                break;
            }
...
입출력 예
n	words	                                                        result
3	["tank", "kick", "know", "wheel", "land", "dream", "mother",
    "robot", "tank"]	                                            [3,3]
